...
impl Goal {
    ...
    pub fn bounding_box(&self) -> Rect {
        // Return a rectangle that tightly surrounds `self.position` and only contains
        // the visible portion of the sprite. You will need to measure the sprite dimensions.
        Rect::from_center(self.position, 50, 60)
    }

    /// Draw the goal onto the given canvas
    pub fn render(&self, canvas: &mut WindowCanvas, textures: &[Texture]) -> Result<(), String> {
        // TODO: The bounding box and the width are kinda weird
        // Copy and paste the render method body from either player.rs or enemy.rs and
        ...
        let (sprite_width, sprite_height) = (40, 75);
        let sprite_x = sprite_width;
        let sprite_y = sprite_height;
